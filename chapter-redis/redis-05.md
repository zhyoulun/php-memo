...
来自[https://github.com/phpredis/phpredis](https://github.com/phpredis/phpredis)
...
### connect, open
-----
_**Description**_: Connects to a Redis instance. 连接到一个redis实例
...
### pconnect, popen
-----
_**Description**_: Connects to a Redis instance or reuse a connection already established with `pconnect`/`popen`. 连接到一个redis实例，或者复用已经建立好的连接
...
### setNx
-----
_**Description**_: Set the string value in argument as value of the key if the key doesn't already exist in the database. 如果键值对不存在，则设置该键值对；如果已存在，则不做任何操作
...
### getSet
-----
_**Description**_: Sets a value and returns the previous entry at that key. 为指定的键设置一个值，并返回这个键先前的值。
##### *Parameters*
*Key*: key
...
### randomKey
-----
_**Description**_: Returns a random key. 随机获取一个键
...
### move
-----
_**Description**_: Moves a key to a different database. 移动一个键到其它数据库中
...
### rename, renameKey
-----
_**Description**_: Renames a key. 重命名一个键
##### *Parameters*
...
### renameNx
-----
_**Description**_: Same as rename, but will not replace a key if the destination already exists. This is the same behaviour as setNx. 和rename的作用一样，但是如果目标键已经存在，将不会替换。这和setnx是一样的。

### expire, setTimeout, pexpire
-----
_**Description**_: Sets an expiration date (a timeout) on an item. pexpire requires a TTL in milliseconds. 给键设置一个过期时间
...
### expireAt, pexpireAt
-----
_**Description**_: Sets an expiration date (a timestamp) on an item. pexpireAt requires a timestamp in milliseconds. 给一个键设置过期时间戳
...
### mSet, mSetNx
-----
_**Description**_: Sets multiple key-value pairs in one atomic command. MSETNX only returns TRUE if all the keys were set (see SETNX). 在一个原子操作中，设置多个键值对。只有当所有的键被设置了，msetnx才会返回true
...
### hIncrBy
-----
_**Description**_: Increments the value of a member from a hash by a given amount. 使用一个指定的数，增加hash中一个member的值
##### *Parameters*
*key*
*member*
...
### hIncrByFloat
-----
_**Description**_: Increments the value of a hash member by the provided float value  使用一个指定的浮点数，增加hash中一个member的值
##### *Parameters*
*key*
*member*
